# Functional Programming: [**What?**](#ll-1) and [**Why?**](#ll-2)
...
## **"composing"**
<p>
Composition means that we can attach multiple functions together, in a chain, where the return value
...
- Compose and pipe are similar. What is change is the order: 
 + Composition: evaluates `inside out, <-right-to-left or `down-top`. 
 + Pipe: `evaluates outside in, left-to-right->` or `top-down`.
 

#### Compose and pipe example:
<p>
If you want to compose two functions you are forced to write your own compose function:
</p>
...
t1 = pipe(n1, n2); // n1, n2
t2 = compose(n1, n2); // n2, n1
```

<p>
or use the already available ones in libraries such as [Lodash/fp](https://gist.github.com/jfmengels/6b973b69c491375117dc "Lodash/fp Docs") and [Ramda](https://ramdajs.com/docs/#compose "Ramda compose")
</p>


#### Compose example using TypeSsript
...
#### RxJS .pipe() operators [pipeable operator](https://github.com/ReactiveX/rxjs/tree/master/src/internal/operators "pipeable operators")
* RxJS Pipe operator return the [Observable](https://github.com/ReactiveX/rxjs/blob/master/src/internal/Observable.ts "github rxjs Observable.ts") result of all of the operators having been called in the order they were passed in.

```javascript
// pipeable operator with Angular6+
...
![Lego]((https://raw.githubusercontent.com/leolanese/FP/master/giphy.gif "FP is like lego")
